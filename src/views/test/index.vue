...
export default defineComponent({
  ...
  setup() {
    ...
    const state = reactive({
      // Table column configuration, most attributes are the same as the `el-table-column` configuration
      columns: [
        ...
        },
        {
          ...
          width: 180,
          align: 'center',
          tdSlot: 'operate', // Custom slot name for the cell content
        },
      ],
      ...
      // paginationConfig: {
      //   layout: 'total, prev, pager, next, sizes', // Pagination component displays which functionalities
      ...
      async getList(params) {
        console.log(params)
        // `params` is received from the component, including pagination and search fields
        const { data } = await getUsers(params)

        // It is required to return an object containing the `data` array and the `total` count
        return {
          data: data.list,
          total: +data.total,
        }
